...
int MX_OPENAMP_Init(int RPMsgRole, rpmsg_ns_bind_cb ns_bind_cb)
{
  ...
  rpmsg_virtio_init_shm_pool(&shpool, (void *)VRING_BUFF_ADDRESS,
                             (size_t)SHM_SIZE - 0x800U);   /* 2026-09-03: the pool starts 0x800 into the region; sizing it
                                                             * with the whole SHM_SIZE let its tail run 2 KB past the region
                                                             * end (into the CM4 breadcrumb / health / process-image header) */
