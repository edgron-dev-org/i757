...
#define ENABLE_SD_DMA_CACHE_MAINTENANCE  1   /* direct edit (registered): AXI cacheable region + IDMA must be maintained */
...
#define ENABLE_SCRATCH_BUFFER   /* direct edit (registered): if off = non-32-aligned buffer passed straight to DMA + full-line cache ops -> corrupts FATFS struct neighbors (FR_INT_ERR case) */
/* USER CODE END enableScratchBuffer */

/* Private variables ---------------------------------------------------------*/
#if defined(ENABLE_SCRATCH_BUFFER)
#if defined (ENABLE_SD_DMA_CACHE_MAINTENANCE)
ALIGN_32BYTES(static uint8_t scratch[BLOCKSIZE]) __attribute__((section(".axisram"))); /* direct edit: SDMMC IDMA cannot reach DTCM (RX_OVERRUN case), moved to AXI */
#else
     ...
     */
     SCB_InvalidateDCache_by_Addr((uint32_t*)scratch, BLOCKSIZE);
      ...
      for (i = 0; i < count; i++)
      {
        ...
        buff += BLOCKSIZE;
#if (ENABLE_SD_DMA_CACHE_MAINTENANCE == 1)
        SCB_CleanDCache_by_Addr((uint32_t*)scratch, BLOCKSIZE);   /* direct edit (registered): ST template defect -- no Clean after memcpy, DMA reads stale RAM and corrupts the card (files-not-persisted case) */
#endif
