...
/* app_dose.h — nutrient/pH dosing control (three peristaltic pumps on the relay module).
 ...
 *   dose a|b|acid <ml>   -> manual timed shot (flow calibration applies)
 *   dose ab <ml>         -> A then B sequentially, <ml> each (the equal-parts SOP)
 *   dose stop            -> stop pumps now, clear queue, mode -> off
 *   dose auto            -> closed loop ON (EC low -> A+B, pH high -> acid)
 *   dose off             -> closed loop off; pumps held OFF (boot default)
 *   dose release         -> release the three coils to the dashboard
 *   dose cal a|b|acid <ml_per_min>   -> pump flow calibration
 *   dose ec <uS> | dose ph <x100> | dose mix <min>  -> auto-loop targets
