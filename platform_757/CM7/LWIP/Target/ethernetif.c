...
#define INTERFACE_THREAD_STACK_SIZE ( 2048 )  /* direct edit (nucleo defect list): ST template 350B overflows and corrupts the tcpip mutex */
...
#define ETH_RX_BUFFER_CNT             24U  /* direct edit (nucleo): public-network OTA block streams burst through the 12-buffer pool */
...
/* USER CODE BEGIN 2 */
ETH_TxPacketConfig TxConfig;  /* direct edit (nucleo defect list): generation defect, definition missing */
...
/* USER CODE END OS_THREAD_ATTR_CMSIS_RTOS_V2 */
  ...
  { extern void app_mac_from_uid(uint8_t *mac6);
    app_mac_from_uid(MACAddr); }   /* ST default 00:80:E1:00:00:00 collides across the whole network + poisons ARP -> derive a unique MAC from the UID */
 ...
 * @param netif the lwip network interface structure for this ethernetif
 */
volatile uint32_t dbg_sem_wake = 0, dbg_rx_frames = 0, dbg_input_err = 0, dbg_alloc_bad = 0; /* direct edit (nucleo diagnostic counters, referenced by app_mqtt heartbeat; not instrumented on 757 yet = always 0) */
