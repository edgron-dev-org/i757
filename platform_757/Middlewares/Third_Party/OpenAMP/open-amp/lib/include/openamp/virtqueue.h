...
#define VQ_RING_ASSERT_VALID_IDX(_vq, _idx)
#define VQ_RING_ASSERT_CHAIN_TERM(_vq)
/* Edgron 2026-09-03: the parameter checks are NOT debug-only. Without them
 * virtqueue_add_buffer() on a full ring (vq_free_cnt == 0, head == CHAIN_END = 0x8000)
 * indexes desc[0x8000] and writes outside SRAM4: imprecise bus fault -> reset. Caught on
 * SWD on the bench board after 1318 field resets in one evening. Keep the check, keep the asserts off. */
#define VQ_PARAM_CHK(condition, status_var, status_err) do { if (((status_var) == 0) && (condition)) { status_var = status_err; } } while (0)
#define VQUEUE_BUSY(vq)
